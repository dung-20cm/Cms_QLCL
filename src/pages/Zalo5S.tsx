...
function clBadge(cl: string) {
  ...
  if (v.includes('trung') || v.includes('kha') || v.includes('khá') || v === 'tb')
...
export default function Zalo5S() {
  ...
  const kpi = useMemo(() => {
    ...
    const tot = daGui.filter((r) => (r.chat_luong || '').toLowerCase().includes('tốt') || r.chat_luong === 'tot').length
    return {
      daGui: daGui.length,
      duSoLuong,
  ...
  return (
    <div>
      <PageHeader
        ...
        actions={
          <>
            <select className={inputCls} value={filterTuan} onChange={(e) => setFilterTuan(e.target.value)}>
            ...
            >
              <Printer size={15} /> {exporting === 'html' ? 'Đang xuất...' : 'Báo cáo HTML/PDF'}
            </button>
            <button
              className={btnSecondary}
            ...
            >
              <FileText size={15} /> {exporting === 'word' ? 'Đang xuất...' : 'Xuất Word'}
            </button>
      ...
      <div className="mb-5 grid grid-cols-2 gap-4 xl:grid-cols-4">
        <KpiCard label="📤 Đã gửi ảnh" value={`${kpi.daGui}/${khoaList.length}`} accent="navy" />
        <KpiCard label="✅ Đủ số lượng (≥3 ảnh)" value={kpi.duSoLuong} accent="green" />
                ...
                {displayedKhoaList.map((k, idx) => {
                  ...
                  return (
                    <tr key={k.id} className="border-b border-gray-50 last:border-0 hover:bg-gray-50/60 dark:border-gray-800 dark:hover:bg-gray-800/40">
                      ...
                      <td className="px-4 py-2.5 font-medium text-gray-700 dark:text-gray-200">{k.ten_khoa}</td>
                      <td className="px-4 py-2.5">
                        {r ? (
                          <span className="rounded-full bg-emerald-50 px-2.5 py-0.5 text-xs font-medium text-emerald-600 dark:bg-emerald-500/10 dark:text-emerald-400">✓ Đã gửi</span>
                        ) : (
                          <span className="rounded-full bg-red-50 px-2.5 py-0.5 text-xs font-medium text-red-500 dark:bg-red-500/10 dark:text-red-400">Chưa gửi</span>
                        )}
                      </td>
                      <td className="px-4 py-2.5 text-gray-500 dark:text-gray-400">
                        {r ? (
                          <span className={r.so_luong_anh >= 3 ? '' : 'font-medium text-amber-600'}>{r.so_luong_anh}</span>
                        ) : '—'}
                      </td>
                      <td className="max-w-[260px] px-4 py-2.5">
                        <div className="flex flex-wrap gap-1">
                          {r?.vi_tri?.map((v) => (
                            <span key={v.id} className="rounded bg-gray-100 px-1.5 py-0.5 text-[10px] text-gray-500 dark:bg-gray-800 dark:text-gray-400">
                      ...
                      <td className="px-4 py-2.5">
                        {r && <span className={`rounded-full px-2 py-0.5 text-xs font-medium ${clBadge(r.chat_luong)}`}>{r.chat_luong}</span>}
                      </td>
                      <td className="max-w-[180px] truncate px-4 py-2.5 text-xs text-gray-400">{r?.ghi_chu}</td>
      ...
      >
        <div className="grid gap-4">
          ...
          <Field label="Số ảnh đã gửi">
            <input type="number" min={0} max={50} className={inputCls} value={mSoAnh} onChange={(e) => setMSoAnh(Number(e.target.value))} />
          </Field>
              ...
              {mConfigTypes.map((v) => {
                ...
                return (
                  <button
                    key={v.id}
                    type="button"
                    onClick={() => setMViTri((p) => (on ? p.filter((x) => x !== v.id) : [...p, v.id]))}
          ...
          <Field label="Chất lượng ảnh">
            <select className={inputCls} value={mChatLuong} onChange={(e) => setMChatLuong(e.target.value)}>
          ...
          <Field label="Ghi chú">
            <input className={inputCls} value={mGhiChu} onChange={(e) => setMGhiChu(e.target.value)} placeholder="VD: Thiếu ảnh tủ thuốc, ảnh buồng bệnh mờ..." />
          </Field>
