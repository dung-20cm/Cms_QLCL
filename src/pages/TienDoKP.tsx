  ...
  const filtered = useMemo(
    () =>
      rows.filter((r) => {
        ...
        if (fKhoa && String(r.danh_gia_chi_tiet?.danh_gia?.khoa_id) !== fKhoa)
          return false;
  ...
  return (
    <div>
      ...
      <div className="mb-5 flex flex-wrap items-end gap-3 rounded-2xl border border-gray-200 bg-white p-4 dark:border-gray-800 dark:bg-gray-900">
        <Field label="Tuần">
          <select
            className={inputCls}
            value={fTuan}
            onChange={(e) => setFTuan(e.target.value)}
          >
      ...
      {loading ? (
        ...
        <EmptyState
          icon={<Wrench size={36} />}
          message="Chưa có hành động khắc phục nào"
        />
      ) : (
                    ...
                    >
                        ...
                        <p className="mt-0.5 text-xs text-gray-400">
                          ...
                          {dgct?.checklist_item && (
                            <span
                              ...
                              style={{
                                background: dgct.checklist_item.s_lt,
                                color: dgct.checklist_item.s_color,
                              }}
                            >
                              {dgct.checklist_item.s_id} ·{" "}
                              {dgct.checklist_item.sub}
                            </span>
                          )}
                        </p>
                      ...
                      <td className="px-4 py-3">
                        {r.trang_thai === "Đã xong" ? (
                          ...
                          <span
                            className={`text-xs font-semibold ${conLai < 0 ? "text-red-500" : conLai <= 2 ? "text-amber-500" : "text-emerald-600 dark:text-emerald-400"}`}
                          >
                            {conLai < 0
                      ...
                      <td className="px-4 py-3">
                        ...
                        {r.ghi_chu && (
                          <p className="mt-1 max-w-[160px] truncate text-[11px] text-gray-400">
                            {r.ghi_chu}
                          </p>
                        )}
                      </td>
          ...
          {editing?.danh_gia_chi_tiet?.checklist_item && (
            <div className="rounded-xl bg-gray-50 p-3 text-xs text-gray-500 dark:bg-gray-800 dark:text-gray-400">
              <span className="font-semibold">
                {editing.danh_gia_chi_tiet.checklist_item.s_id}
              </span>{" "}
              · {editing.danh_gia_chi_tiet.checklist_item.tc}
            </div>
          )}
          ...
          <div className="grid grid-cols-2 gap-4">
            <Field label="Người phụ trách">
              <select
                ...
                value={mNguoi}
                onChange={(e) =>
                  setMNguoi(e.target.value ? Number(e.target.value) : "")
                }
              >
            ...
            <Field label="Hạn xử lý">
              <input
                ...
                className={inputCls}
                value={mHan}
                onChange={(e) => setMHan(e.target.value)}
              />
            </Field>
          </div>
          <Field label="Trạng thái">
